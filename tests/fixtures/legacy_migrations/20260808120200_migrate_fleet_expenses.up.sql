...
--
-- GUARDED on the source table existing. Two situations require this:
--   1. After the follow-up migration drops public.fleet_expenses, any fresh
--      deploy replays the whole migration chain -- an unguarded reference would
--      make this migration fail forever on a new database.
--   2. Ephemeral test databases have the banksms schema but none of the legacy
--      `public` tables.
-- In both cases the correct behaviour is "nothing to migrate", not an error.
--
...
BEGIN
    IF to_regclass('public.fleet_expenses') IS NULL THEN
        RAISE NOTICE 'public.fleet_expenses does not exist; nothing to migrate';
        RETURN;
    END IF;

    INSERT INTO banksms.transactions (
        source, import_source_id,
        parsed_direction, parsed_amount, parsed_currency, parsed_occurred_at,
    ...
    SELECT
        ...
        -- Hand-entered, not machine-parsed. parser_version 0 marks "no parser
        -- ran", which keeps these rows outside every re-parse scope (which is
        -- additionally guarded on source = 'whatsapp').
        ...
        -- The "categories" carried over verbatim: Other, Labor, Parts,
        -- Registration, Fuel, Maintenance, Repairs, Insurance.
        fe.expense_type,
    ...
    -- Fail loudly if anything was silently dropped or the money moved.

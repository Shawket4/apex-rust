...
pub async fn get_session_location_pings(
    ...
    let (pings, original_count) = if downsample_target > 0
        && known_count > SERVER_DOWNSAMPLE_THRESHOLD
    {
        let pings =
            fetch_location_pings_downsampled(pool.get_ref(), session_id, downsample_target as i32)
                .await
                .map_err(actix_web::error::ErrorInternalServerError)?;
    ...
    let response_data = LocationPingsResponse {
        ...
        stats,
        total_pings: if include_total_pings {
            total_pings
        } else {
            None
        },
